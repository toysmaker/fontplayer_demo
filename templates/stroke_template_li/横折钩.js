...
const getComponents = (skeleton) => {
  ...
  const out_radius_start_zhe_gou = getRadiusPoint({
    ...
    radius: Math.min(out_radius_zhe_gou * 2, FP.distance(zhe_start, zhe_end) * 0.5),
  })
  const out_radius_end_zhe_gou = getRadiusPoint({
    start: out_gou_start,
    end: out_gou_end,
    radius: Math.min(out_radius_zhe_gou * 2, FP.distance(gou_start, gou_end) * 0.5),
  })

  const end_p0 = out_radius_start_zhe_gou
  const end_p3 = out_radius_end_zhe_gou
  const end_p1 = FP.getPointOnLine(out_radius_start_zhe_gou, out_corner_zhe_gou, FP.distance(out_radius_start_zhe_gou, out_corner_zhe_gou) * 0.5)
  const end_p2 = FP.getPointOnLine(out_radius_end_zhe_gou, out_corner_zhe_gou, FP.distance(out_radius_end_zhe_gou, out_corner_zhe_gou) * 0.5)
  const end_p4 = out_gou_end
  const end_p5 = in_gou_end
  const end_p4_radius_before = FP.getPointOnLine(end_p4, end_p3, radius)
  const end_p5_radius_after = FP.getPointOnLine(end_p5, in_corner_zhe_gou, radius)
