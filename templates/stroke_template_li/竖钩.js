...
const getComponents = (skeleton) => {
  ...
  let { corner: in_corner_shu_gou } = FP.getIntersection(
    { type: 'line', start: in_shu_start, end: in_shu_end },
    { type: 'line', start: in_gou_start, end: in_gou_end },
  )
  let { corner: out_corner_shu_gou } = FP.getIntersection(
  ...
  const control_length = Math.min(l * 0.5 - start_length, l * 0.55 - end_length, 45)
  const p15 = FP.turnLeft(out_shu_end, FP.getPointOnLine(out_shu_end, out_shu_start, l * 0.55 - control_length), d)
  const p16 = FP.turnLeft(out_shu_end, FP.getPointOnLine(out_shu_end, out_shu_start, l * 0.55), d)
  const p17 = FP.turnLeft(out_shu_end, FP.getPointOnLine(out_shu_end, out_shu_start, l * 0.55 + control_length), d)
  const p6 = FP.turnRight(in_shu_end, FP.getPointOnLine(in_shu_end, in_shu_start, l * 0.55 - control_length), d)
  const p5 = FP.turnRight(in_shu_end, FP.getPointOnLine(in_shu_end, in_shu_start, l * 0.55), d)
  const p4 = FP.turnRight(in_shu_end, FP.getPointOnLine(in_shu_end, in_shu_start, l * 0.55 + control_length), d)
  ...
  out_corner_shu_gou = FP.getIntersection(
    { type: 'line', start: p13, end: p12 },
    { type: 'line', start: out_gou_start, end: out_gou_end },
  ).corner
  const end_p5 = p8
  const end_p4 = out_gou_end
  const end_p6 = p7
  const end_p6_radius_before = p7_radius_before
  const end_p6_radius_after = p7_radius_after
  const end_p0 = FP.getPointOnLine(out_corner_shu_gou, p13, FP.distance(out_corner_shu_gou, p13) * 0.7)
  const end_p1 = FP.getPointOnLine(out_corner_shu_gou, p13, FP.distance(out_corner_shu_gou, p13) * 0.35)
  const end_p2 = FP.getPointOnLine(out_corner_shu_gou, end_p4, Math.min(FP.distance(out_corner_shu_gou, end_p4) * 0.35, out_radius))
  const end_p3 = FP.getPointOnLine(out_corner_shu_gou, end_p4, Math.min(FP.distance(out_corner_shu_gou, end_p4) * 0.7, out_radius * 2))
  const end_p4_radius_before = FP.getPointOnLine(end_p4, end_p3, radius)
  const end_p5_radius_after = FP.getPointOnLine(end_p5, end_p6, radius)

  const end_p5_joint = new FP.Joint('end_p5_joint', {
    x: end_p5.x,
    y: end_p5.y,
  })
  const end_p6_joint = new FP.Joint('end_p6_joint', {
    x: end_p6.x,
    y: end_p6.y,
  })
